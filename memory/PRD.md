...
- **Iteration 11 (2026-02) — Workspace wrap-up:** (1) Fixed all 1997/1998 freedom dates so they map exactly to `launch_year + 30` (same month/day) — corrected 21 legacy CSV rows + 22 live Mongo docs. Pending countdown count: **29 → 50**. (2) Added expandable `<VRTDisclaimer />` section directly below the VRT badge on every car detail page covering: €200 Category C VRT + zero NOx levy, GB import (10% Customs Duty, 23%/13.5% VAT), NI import (V5C + Windsor Framework exemption). Verified live on 1997 BMW M3 E36 Evolution.
